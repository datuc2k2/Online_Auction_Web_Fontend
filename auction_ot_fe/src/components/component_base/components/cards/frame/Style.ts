...
import { ICards } from './CardsFrame';
import { IGlobalTheme } from '../../../types/Global';

const BtnWraper = (theme: IGlobalTheme) => `
    .ant-card-head {
      display: flex;
...
export const CardFrame = Styled(Card)<ICards>`

  ${({ isbutton, theme }: { isbutton?: ReactNode; theme: IGlobalTheme }) => isbutton && BtnWraper(theme)}
  ...
  .ant-card-body{
    padding: ${({ bodypadding }: { bodypadding?: string }) => (bodypadding ? `${bodypadding} !important` : '25px !important')};
    @media only screen and (max-width: 575px){
      padding: ${({ bodypadding }: { bodypadding?: string }) => (bodypadding ? `${bodypadding} !important` : '15px !important')};
    }
    table{
      .ant-tag{
        line-height: 18px;
