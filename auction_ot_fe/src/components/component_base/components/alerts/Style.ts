...
const outline = (theme: IGlobalTheme, type?: $PropertyType<AlertProps, 'type'>) => {
  return `
    ...
    .ant-alert-message, .ant-alert-message {
      color: ${({ theme }: { theme: IGlobalTheme }) => theme[`${type}-color`]};
    }
  `;
};

export const AlertWrap = Styled(Alert) <IAlert>`
